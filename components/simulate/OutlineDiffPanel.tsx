      ...
      {changeCount > 0 && (
        <div className="px-3 py-2 border-t bg-muted/30 flex-shrink-0 space-y-2">
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-primary-foreground bg-primary hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed rounded transition-colors"
          >
            <Check className="h-3.5 w-3.5" />
            <div className="flex-1 text-left">
              <div>Change Outline & Propose</div>
              <div className="text-xs text-primary-foreground/70 font-normal">Others can see and modify</div>
            </div>
          </button>
          <button
            onClick={() => onStartDiscussion(selectedIds, mergedChildren)}
            disabled={selectedCount === 0}
            className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-foreground bg-secondary hover:bg-secondary/80 disabled:opacity-50 disabled:cursor-not-allowed border border-border rounded transition-colors"
          >
            <MessageSquare className="h-3.5 w-3.5" />
            <div className="flex-1 text-left">
              <div>Start Discussion</div>
              <div className="text-xs text-muted-foreground font-normal">Requires input from others</div>
            </div>
          </button>
        </div>
      )}
