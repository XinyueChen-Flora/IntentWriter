    ...
    <div className="border rounded-lg overflow-hidden">
      ...
      >
        ...
        {impact.impactLevel === 'significant' && (
          <span className="px-1.5 py-0.5 text-xs font-medium rounded bg-primary/10 text-primary dark:bg-primary/20">
            Significant
          </span>
        )}
        {impact.impactLevel === 'none' && (
          <span className="px-1.5 py-0.5 text-xs font-medium rounded bg-muted/50 text-muted-foreground/70">
            No change
          </span>
        )}
      </button>
        ...
        <div className="px-3 py-2 border-t bg-muted/20 space-y-1">
          {mergedOutline.length === 0 ? (
            ...
            mergedOutline.map(item => (
              ...
              >
                {item.status === 'new' && <Plus className="h-3 w-3 text-muted-foreground flex-shrink-0 mt-0.5" />}
