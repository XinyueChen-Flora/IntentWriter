...
export function WritingSectionPanel({ block, children }: WritingSectionPanelProps) {
  ...
  // ── Auto-detect pending proposals for reviewer ──
  // Two detection paths:
  // 1. Block-based: changeStatus='proposed' (Team Vote, Discussion — pending approval)
  // 2. Notification-based: unacknowledged notification (Inform — already applied, needs ack)
  const pendingProposal = useMemo(() => {
    // Path 1: block with proposed status (pending approval)
    const proposedChild = children.find(
      c => c.changeStatus === 'proposed' && c.proposalId && c.changeBy !== ctx.currentUser.id
    );
    if (proposedChild) {
      ...
      return {
        ...
        pathId: notification?.proposeType || 'decided',
        proposedBy: proposedChild.changeByName || 'Someone',
      };
    }

    // Path 2: unacknowledged notification (Inform — changes already applied)
    const notifications = ctx.getSectionNotifications(block.id);
    const unacked = notifications.find(n => !n.acknowledged && n.notifyLevel !== 'skip');
    if (unacked) {
      return {
        proposalId: unacked.proposalId,
        pathId: unacked.proposeType || 'decided',
        proposedBy: unacked.proposedByName || 'Someone',
        notification: unacked, // pass full notification data so deliberate functions can read it
      };
    }
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-row items-stretch">
      ...
      {/* ── Negotiate Runner Panel — after Gate routes to a negotiate protocol ── */}
      {activeNegotiateDef && (
        <div className="border rounded-lg overflow-hidden flex flex-col w-80 flex-shrink-0 bg-card">
          <div className="flex items-center justify-between px-3 py-2 border-b bg-muted/20">
      ...
      <div className="flex-1 min-w-0 border rounded-lg bg-card overflow-hidden flex flex-col">
        ...
        {/* Pending proposal notification — click to open deliberate panel */}
        {pendingProposal && !activeNegotiateId && (
          <button
            onClick={() => {
              ...
              if (pathDef) {
                ...
                setNegotiateConfig({
                  ...
                  notification: (pendingProposal as any).notification || null,
                });
              }
            }}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <span className="text-xs font-medium text-amber-800">
                {pendingProposal.proposedBy} changed {(pendingProposal as any).notification?.sourceSectionName || 'a section'}
              </span>
